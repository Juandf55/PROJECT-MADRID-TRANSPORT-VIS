...
def fill_gdf_madrid_boundaries():
    ...
    try:
        ...
        gdf_madrid_boundaries = gdf_madrid_boundaries.to_crs(
            epsg=3857)  # transformation to epsg 3857 standard for all of my maps.
...
xx, yy = np.mgrid[xmin:xmax:complex(0, RESOLUTION),
         ymin:ymax:complex(0, RESOLUTION)]  # two main matrix, [500 * 500] dividing the space in parts
positions = np.vstack(
    [xx.ravel(), yy.ravel()])  # one "inline" array for xx and other for yy with all the divisions : [2, 50.000]
...
gdf_choropleth['pop_density_log'] = np.log1p(
    gdf_choropleth['pop_density'])  # Normalize Population Density (Log Normalization 0-1)
...
for i, target_name in enumerate(MODE_NAMES):
    ...
    feature_imp_df = pd.DataFrame({'Feature': X.columns, 'Importance': importances}).sort_values(by='Importance',
                                                                                                 ascending=False)
