...
print(":::::::::::::::::::::::::::::::::::::::::::::::::::: " + "\n")
print("GDF_CHOROPLETH INFO :", gdf_choropleth.info())
...
RESOLUTION = 200 #in how many pixeles we will observe the KDE
...
def calculate_spatial_features(gdf_ml_grid, xmin, xmax, ymin, ymax):
    ...
    # Ensure grid shape matches
    # Note: create_ml_grid uses np.arange which might result in slightly different shape if not perfectly divisible
    # We should rely on the actual shape of the dataframe if it was a perfect grid.
    # Let's assume the grid generation order (cols then rows) matches reshaping (cols, rows) or (rows, cols).
    # create_ml_grid: for x in cols: for y in rows. This is column-major order if we reshape to (len(cols), len(rows)).
    
    # Re-derive shape from unique coordinates to be safe
    ...
    # We need a matrix of TOTAL STOPS (all modes)
    # First, sum all count columns
    mode_cols = [c for c in gdf_ml_grid.columns if c.startswith('count_')]
    total_stops_series = gdf_ml_grid[mode_cols].sum(axis=1)
    
    # Reshape to grid
    # The grid was created: x (outer), y (inner). 
    # So the first len(unique_y) elements are for the first x.
    # This corresponds to shape (len(unique_x), len(unique_y)).
    grid_matrix = total_stops_series.values.reshape(shape)
    
    # Kernels
    # 3x3 neighbors (sum 8 neighbors)
    k3 = np.ones((3, 3))
    k3[1, 1] = 0
    
    # 5x5 neighbors (sum 24 neighbors)
    k5 = np.ones((5, 5))
    k5[2, 2] = 0 # Center
    # Note: The prompt says "24 celdas circundantes". A 5x5 square has 25 cells. Minus center = 24.
    # So strictly the 5x5 box around the center.
    
    # Convolve
    # mode='constant', cval=0 assumes 0 stops outside the map
