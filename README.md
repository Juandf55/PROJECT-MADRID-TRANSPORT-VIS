...
DATA COLLECTION & VISUALIZATION Author: Juan de Frutos
...
Code Explanation (Kernel Logic & Math)

The KDE algorithm aggregates the influence of individual transport stops to estimate a continuous density surface across the Madrid grid.
...
Loops 5 times. In each iteration, one transport mode is the Target (Y), and the others + Population are predictors (X).
...
ML Model 3: Balanced Baseline Configuration
...
Final Conclusion: Model 2 (Granular/Multiple Columns) is the superior model for urban planning. It demonstrates that when we account for the specific type of transport available, the actual
